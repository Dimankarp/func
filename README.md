...
## Поддержка LLVM
Изначально `FunC` разрабатывался под [эмулятор `risc` процессора](https://asurkis.github.io/risc-emulator/). В новой версии компилятор `FunC` может генерировать LLVM IR код и компилировать его под `x86` архитектуру. Отличительные особенности `FunC` при генерации LLVM IR кода, которые не поддерживаются в режиме эмулятора:
...
- Программа может быть разбита на несколько файлов;
   - Пропало требование об обязательном наличии функция `(void-void) main` 
- Доступна [стандартная библиотека](./stdlib/stdfunc.asm) с системными вызовами и набор [вспомогательных функций](./stdlib/utils.fc);
- Выражение `return` в `void` функциях может быть опущен;
- Появилась проверка типа результата в выражении с `return`;
- Любые типы (в том числе и функциональные) поддерживают операции `==` и `!=`.

Также для режима с LLVM IR доступны [тесты](./tests/). Для из запуска необходимо после сборки компилятора выполнить команду:
```bash
